...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()                                                #stores the probability of a ghost at that position        
        
        #When noisy distance is none the ghost is eaten and thrown to jail, so we are updating that possible value to 1 
        if(noisyDistance==None):
            allPossible[self.getJailPosition()]=1
        else:
            for p in self.legalPositions:                                           #Looping through all possible Ghost positions
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    allPossible[p] = self.beliefs[p] * emissionModel[trueDistance]  #Updating allPossible to currentBelief * observation

            allPossible.normalize()                                                 #normalizing and updating pacman's belief
        
        self.beliefs = allPossible                                                  #Updating belief to allPossible
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        allPossible = util.Counter()                                                #stores the probability of a ghost at that position
        for oldPositions in self.legalPositions:                                    #looping over all possible Ghost positions
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPositions)) #For each possible Ghost position we are calculating the distribution
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += self.beliefs[oldPositions] * prob            #Updating allPossible future position for Ghost based on belief 
                                                                                    #for Ghost's current position and probability of the future position
        
        allPossible.normalize()                                                     #Normalizing allPossible 
        
        self.beliefs = allPossible                                                  #Updating pacman's belief for Ghost positions
